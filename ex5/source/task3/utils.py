...
def plot_phase_portrait_nonlinear(C, basis, X, Y, ep):
    """
    Plots a non linear vector field in a streamplot, defined with X and Y coordinates and the matrix A.
    ...
    plotting of the result.
    """
    data = np.row_stack([X.ravel(), Y.ravel()])
    phi = np.exp(-((data.T.reshape(-1, 1, 2) - basis.reshape(-1, 2)) ** 2).sum(-1) / ep)
    UV = (phi @ C).T
    U = UV[0,:].reshape(X.shape)
    V = UV[1,:].reshape(X.shape)
